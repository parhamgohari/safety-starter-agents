...
"""
Actor-Critics to learn immediate rewards and cost
"""
def mlp_actor_critic_im(state_action, hidden_sizes=(64,64), activation=tf.tanh,
                     output_activation=None, policy=None, action_space=None):
    ...
    with tf.variable_scope('r_im'):
        rewQ = tf.squeeze(mlp(state_action, list(hidden_sizes)+[1], activation, None), axis=1)

    with tf.variable_scope('c_im'):
        costQ = tf.squeeze(mlp(state_action, list(hidden_sizes)+[1], activation, None), axis=1)
    return rewQ, costQ
